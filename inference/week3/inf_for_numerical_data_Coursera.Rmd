...
ggplot(nc %>% filter(habit == 'nonsmoker'), aes(weight)) + geom_density()
ggplot(nc %>% filter(habit == 'smoker'), aes(weight)) + geom_density()
...
inference(y = weeks, x = mature, data = nc, statistic = "mean", type = "ci", 
          method = "theoretical", conf_level = 0.90, order = c("younger mom", "mature mom"))

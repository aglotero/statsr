...
samp %>%
  summarise(lower = params > mean(area) - z_star_95 * (sd(area) / sqrt(n)),
            upper = params < mean(area) + z_star_95 * (sd(area) / sqrt(n)))
...
ci2_data <- data.frame(ci_id = c(1:50, 1:50),
                      ...
                      capture_mu = c(ci2$capture, ci2$capture))

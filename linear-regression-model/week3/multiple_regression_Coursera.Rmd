...
<ol>
<li> The median of the distribution is 4.3. TRUE </li>
<li> **25\% of the students gave their professors a score of over 4.6.** FALSE </li> 
<li> 11 of students gave a professor a score below 3. TRUE</li> 
...
ggplot(data = model, aes(x = model$fitted.values, y = model$residuals)) +
  geom_point()
```
...
m_sem_bty_avg <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits 
             + pic_outfit + pic_color, data = evals)
summary(m_sem_bty_avg)
#0.1538

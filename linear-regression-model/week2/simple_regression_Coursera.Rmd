...
ggplot(mlb11, aes(y = runs, x = at_bats)) + geom_point()
...
mhr <- lm(runs ~ homeruns , data = mlb11)
...
residual = observedValue - (−2789.2429+0.6305*5579)
residual
```
...
mh_stolen_bases <- lm(runs ~ stolen_bases , data = mlb11)
summary(mh_stolen_bases)

ggplot(data = mlb11, aes(x = stolen_bases, y = runs)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)

ggplot(data = mh_stolen_bases, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = mh_stolen_bases, aes(sample = .resid)) +
  stat_qq()
...
mh_x <- lm(runs ~ bat_avg , data = mlb11)
summary(mh_x)
...
ggplot(data = mh_x, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")
...
mh_x1 <- lm(runs ~ new_onbase , data = mlb11)
summary(mh_x1)
...
ggplot(data = mh_x1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = mh_x1, aes(sample = .resid)) +
  stat_qq()

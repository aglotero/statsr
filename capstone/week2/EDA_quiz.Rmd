...
```{r load}
setwd("~/coursera/statistics/capstone/week2/")
...
ames_train %>% 
  select(Misc.Feature, Fence, Pool.QC, Alley, Fireplace.Qu, Lot.Frontage) %>%
  mutate(Misc.Feature_1 = ifelse(is.na(Misc.Feature),1,0),
         Fence_1        = ifelse(is.na(Fence),1,0),
         Pool.QC_1      = ifelse(is.na(Pool.QC),1,0),
         Alley_1        = ifelse(is.na(Alley),1,0), 
         Fireplace.Qu_1 = ifelse(is.na(Fireplace.Qu),1,0),
         Lot.Frontage_1 = ifelse(is.na(Lot.Frontage),1,0)) %>%
  select(Misc.Feature_1, Fence_1, Pool.QC_1, Alley_1, Fireplace.Qu_1, Lot.Frontage_1) %>%
  as.data.frame() %>%
  as.matrix() %>%
  colSums()
...
str(ames_train)
#Mo.Sold
levels(factor(ames_train$Mo.Sold))
#Yr.Sold
levels(factor(ames_train$Yr.Sold))
```
...
ames_train %>%
  filter(Neighborhood %in% c('StoneBr', 'Timber', 'Veenker', 'NridgHt' )) %>%
  select(Neighborhood, price) %>%
  group_by(Neighborhood) %>%
  summarise(a = sd(price))
...
ggplot(ames_train, aes(y = price, x = Lot.Area)) +
  geom_point() + 
...
ggplot(ames_train, aes(x = log(Lot.Area), y = log(price))) +
  geom_point() +   
      geom_smooth(method=lm,   # Add linear regression lines
                se=FALSE,    # Don't add shaded confidence region
                fullrange=TRUE) # Extend regression lines
...
ames_train %>%
  ...
  mutate(resp = median > mean)
#NO!

#21 houses do not have a basement.
ames_train %>%
  select(Total.Bsmt.SF) %>%
...
data <- ames_train %>%
  select(area, Garage.Area) %>%
  filter(!is.na(Garage.Area), !is.na(area)) %>%
  mutate(hasGarage = factor(ifelse(Garage.Area > 0, "yes", "no")))

inference(data, x = hasGarage, y = area, type = "ht", statistic = "mean", method = "simulation", alternative = "twosided")
...
ggplot(ames_train, aes(y = log(Lot.Area), x = log(price))) +
  geom_point() +   
      geom_smooth(method=lm,   # Add linear regression lines
                se=FALSE,    # Don't add shaded confidence region
                fullrange=TRUE) # Extend regression lines

model = lm(data = ames_train, formula = log(price) ~ log(Lot.Area))
...
ames_train %>%
  ...
  group_by(Neighborhood) %>%
  summarise(a = mean(Bldg.Type == "1Fam")) %>%
  filter(a == 1)
```

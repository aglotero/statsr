...
```{r load}
setwd("~/coursera/statistics/capstone/week5/")
...
model <- lm (data = ames_train, 
             formula = log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr+ Overall.Qual + Land.Slope )

MASS::stepAIC(model)
n<-nrow(ames_train)
MASS::stepAIC(model, k = log(n))
...
model <- lm(data=ames_train,
            formula = formula(log(price) ~ log(area)))

data <- data.frame(residuals = model$residual, Neighborhood = ames_train$Neighborhood)

data %>%
  group_by(Neighborhood) %>%
  summarise(residuals_mean = mean(residuals)) %>%
  arrange(desc(residuals_mean)) %>%
  head(n = 3)
...
data %>%
  group_by(Neighborhood) %>%
  summarise(residuals_mean = mean(residuals)^2) %>%
  arrange(desc(residuals_mean)) %>%
  head(n = 3)
...
(ames_train %>%
  dplyr::select(Overall.Qual, Bsmt.Qual, Garage.Qual) %>%
  filter(!is.na(Overall.Qual), !is.na(Bsmt.Qual), !is.na(Garage.Qual)) %>%
  nrow()
-
ames_train %>% nrow())
...
model <- lm(data=ames_train,
          formula = log(price) ~ Overall.Cond + Overall.Qual)

data <- data.frame(fitted = as.data.frame(model$fitted.values),
                   subclass = ames_train$MS.SubClass)

data %>%
  group_by(subclass) %>%
  summarise(fitted_mean = mean(model.fitted.values)) %>%
...
ames_train_sub <- ames_train %>% filter(Sale.Condition == 'Normal')

model_sub <- lm(data=ames_train_sub,
            formula = formula(log(price) ~ log(area)))

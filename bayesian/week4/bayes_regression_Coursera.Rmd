...
m_lwage_iq = bas.lm(lwage ~ iq, data = wage,
                    prior="BIC",
                    bestmodel=c(1,1),
                    n.models=1)
# use BIC as posterior distribution of coefficients uses OLS estimates
round(confint(coef(m_lwage_iq), level = 0.90), 6)
...
round(quantile(y_5, c(0.025, .975)), 2)
```
